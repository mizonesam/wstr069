...
cJSON *cJSONUtils_GetPointer(cJSON *object,const char *pointer)
{
    while (*pointer++=='/' && object)
    {
        ...
        {
            object=object->child;
            while (object && cJSONUtils_Pstrcasecmp(object->string,pointer)) object=object->next;	/* GetObjectItem. */
...
{
    char *parentptr=0,*childptr=0;
    cJSON *parent=0,*ret=0;
...
static int cJSONUtils_ApplyPatch(cJSON *object,cJSON *patch)
{
    ...
    if (opcode==3 || opcode==4)	/* Copy/Move uses "from". */
    {
        ...
        if (!from) return 4; /* missing "from" for copy/move. */
    ...
    if (!parent)
    {
        free(parentptr);    /* Couldn't find object to add to. */
        return 9;
    }
...
static cJSON *cJSONUtils_SortList(cJSON *list)
{
    ...
    while (ptr)
    {
        second=second->next;    /* Walk two pointers to find the middle. */
        ptr=ptr->next;
        if (ptr) ptr=ptr->next;
    }
    ...
    if (first)
    {
        if (!list) return first;	   /* Append any tails. */
        ptr->next=first;
        first->prev=ptr;
    }
    if (second)
    {
        if (!list) return second;
        ptr->next=second;
        second->prev=ptr;
    }
