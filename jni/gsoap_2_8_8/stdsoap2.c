...
#endif
  SOAP_DPrint("[%s] socket:%d, n:%d\n", __FUNCTION__, soap->socket, n);//linjieling
  while (n)
  { if (soap_valid_socket(soap->socket))
    {
      SOAP_DPrint("[%s] send_timeout:%d\n", __FUNCTION__, soap->send_timeout);//linjieling
      ...
      { if (soap->peerlen)
        ...
        SOAP_DPrint("[%s] nwritten:%d\n", __FUNCTION__, nwritten);//linjieling
#else
...
soap_push_namespace(struct soap *soap, const char *id, const char *ns)
{
  ...
  { for (i = 0; p->id; p++, i++)
    {
      //SOAP_DPrint("[%s] i1:%d\n", __FUNCTION__, i);//linjieling
      //SOAP_DPrint("[%s] p->id:%s, p->ns:%s, p->in:%s, p->out:%s, ns:%s\n",
      //  __FUNCTION__, p->id, p->ns, p->in, p->out, ns);//linjieling
...
{
    //SOAP_DPrint("[%s] --->\n", __FUNCTION__);//linjieling
    //SOAP_DPrint("[%s] n1:%ld, n2:%ld\n", __FUNCTION__, n1, n2);//linjieling
...
#else
    while (np && (strncmp(np->id, id1, n1) || np->id[n1])){
        //SOAP_DPrint("[%s] np->index:%d\n", __FUNCTION__, np->index);//linjieling
        //SOAP_DPrint("[%s] np->id:%s\n", __FUNCTION__, np->id);//linjieling
        //SOAP_DPrint("[%s] np->id[n1]:%c\n", __FUNCTION__, np->id[n1]);//linjieling
          np = np->next;
    }
#endif
    //SOAP_DPrint("[%s] 1\n", __FUNCTION__);//linjieling
  if (np)
  {
    ...
    if (!(soap->mode & SOAP_XML_IGNORENS)){
      if (np->index < 0
       || ((s = soap->local_namespaces[np->index].id) && (strncmp(s, id2, n2) || (s[n2] && s[n2] != '_'))))
      {
        //SOAP_DPrint("[%s] index:%d\n", __FUNCTION__, np->index);//linjieling
        //SOAP_DPrint("[%s] s:%s\n", __FUNCTION__, s);//linjieling
        //SOAP_DPrint("[%s] id2:%s\n", __FUNCTION__, id2);//linjieling
        //SOAP_DPrint("[%s] flag:%d\n", __FUNCTION__, (s[n2] && s[n2] != '_'));//linjieling
        return SOAP_NAMESPACE;
      }  
    }
#endif
...
{
    //SOAP_DPrint("[%s] --->\n", __FUNCTION__);//linjieling
    //SOAP_DPrint("[%s] Tag1:'%s', Tag2:'%s'\n", __FUNCTION__, tag1, tag2);//linjieling 
  ...
  s = strchr(tag1, ':');
  t = strchr(tag2, ':');
      //SOAP_DPrint("[%s] s:'%s', t:'%s'\n", __FUNCTION__, s, t);//linjieling 
  if (t)
  { if (s)
    { if (t[1] && SOAP_STRCMP(s + 1, t + 1))
      ...
      { 
          //SOAP_DPrint("[%s] Tags '%s' and '%s' match but namespaces differ\n", __FUNCTION__, tag1, tag2);//linjieling
    ...
    //SOAP_DPrint("[%s] Tags and (default) namespaces match: '%s' '%s'\n", __FUNCTION__, tag1, tag2);//linjieling    
...
{ if (*soap->arrayType)
    ...
    { DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Array type mismatch: '%s' '%s'\n", soap->arrayType, type));
      SOAP_DPrint("[%s] Array type mismatch: '%s' '%s'\n", __FUNCTION__, soap->arrayType, type);//linjieling
      return SOAP_TAG_MISMATCH;
    }
  return SOAP_OK;
}
...
{ soap_int32 iadd = -1;
  ...
  { host = NULL;
   ...
   host = (struct hostent *)gethostbyname_r(addr, &hostent, soap->buf, SOAP_BUFLEN, NULL, &soap->errnum);
#elif defined(VXWORKS)
...
{ int n = (int)sizeof(soap->peer);
  ...
  soap->errmode = 0;
  soap->keep_alive = 0;
  SOAP_DPrint("[%s] soap->master:%d\n", __FUNCTION__, soap->master);
  if (!soap_valid_socket(soap->master))
  { soap->errnum = 0;
...
soap_done(struct soap *soap)
{
  ...
  { register struct soap_plugin *p = soap->plugins->next;
    ...
    //SOAP_DPrint("[%s] Removing plugin '%s'\n", __FUNCTION__, soap->plugins->id);//linjieling
...
http_parse(struct soap *soap)
{
  ...
  *soap->endpoint = '\0';
  ...
  { if (soap_getline(soap, soap->msgbuf, sizeof(soap->msgbuf)))
    ...
    SOAP_DPrint("[%s] HTTP status: %s\n", __FUNCTION__, soap->msgbuf);//linjieling
    for (;;)
    { if (soap_getline(soap, header, SOAP_HDRLEN))
      ...
      SOAP_DPrint("[%s] HTTP header: %s\n", __FUNCTION__, header);//linjieling
      s = strchr(header, ':');
      if (s)
      { char *t;
        *s = '\0';
  ...
  //SOAP_DPrint("[%s] status1:%d\n", __FUNCTION__, soap->status);//linjieling
  if (soap->status == 0)
  { size_t l = 0;
    if (s)
  ...
  /* Status OK (HTTP 200) */
  //SOAP_DPrint("[%s] status2:%d\n", __FUNCTION__, soap->status);//linjieling
  //if (soap->status == 0 || soap->status == 200 || soap->status == 204)
...
http_post(struct soap *soap, const char *endpoint, const char *host, int port, const char *path, const char *action, size_t count)
{
  ...
  {
	 if ((err = soap->fposthdr(soap, "User-Agent", "HMW tr069-cwmp")))
	 	return err;
  }
  else
  {
    if ((err = soap->fposthdr(soap, "User-Agent", "sy/2.8")))
		return err;
  }
...
#endif
#ifndef WITH_LEAN
  SOAP_DPrint("[%s] userid:%s, passwd:%s\n", __FUNCTION__, soap->userid, soap->passwd);
  ...
  SOAP_DPrint("[%s] proxy_userid:%s, proxy_userid:%s\n", __FUNCTION__, soap->proxy_userid, soap->proxy_userid);
...
http_response(struct soap *soap, int status, size_t count)
{
  ...
  if ((err = soap->fposthdr(soap, "Server", "sy/2.8"))
...
soap_putsetcookies(struct soap *soap)
{
  ...
  for (p = soap->cookies; p; p = p->next)
  {
    ...
    { s = tmp;
      ...
      SOAP_DPrint("[%s] Set-Cookie: %s\n", __FUNCTION__, tmp);//linjieling
      if ((soap->error = soap->fposthdr(soap, "Set-Cookie", tmp)))
        return soap->error;
    }
  }
#if 0
    if ((soap->error = soap->fposthdr(soap, "Set-Cookie", "JSESSIONID=6D3A56D7F6AD187A2716B0F657517B7E; Path=/ACS-server")))
    return soap->error;
#endif
  SOAP_DPrint("[%s] <---\n", __FUNCTION__);//linjieling
  return SOAP_OK;
...
soap_putcookies(struct soap *soap, const char *domain, const char *path, int secure)
{
  ...
  //SOAP_DPrint("[%s] Sending cookies for domain=%s path=%s\n", __FUNCTION__, domain, path);//linjieling
  if (*path == '/')
    path++;
  //SOAP_DPrint("[%s] q:%p, *p:%p\n", __FUNCTION__, q, *p);//linjieling
  while ((q = *p))
  { if (q->expire && now > q->expire)
    { DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Cookie %s expired\n", q->name));
      //SOAP_DPrint("[%s] Cookie %s expired\n", __FUNCTION__, q->name);//linjieling
      SOAP_FREE(soap, q->name);
      if (q->value)
        SOAP_FREE(soap, q->value);
      if (q->domain)
	...
	{ strcat(s, " ");
	  s++;
        ...
        //SOAP_DPrint("[%s] name:%s, value:%s, path:%s\n", __FUNCTION__, q->name, q->value, q->path);//linjieling
        if (q->name)
  ...
  //SOAP_DPrint("[%s] tmp1:%p, s1:%p\n", __FUNCTION__, tmp, s);//linjieling
  if (s != tmp)
    if ((soap->error = soap->fposthdr(soap, "Cookie", tmp)))
      return soap->error;
  //SOAP_DPrint("[%s] <---\n", __FUNCTION__);//linjieling
...
{ register int i;
  ...
  //SOAP_DPrint("[%s] version:%d\n", __FUNCTION__, soap->version);//linjieling
  if (soap->version == 2)
    soap->encoding = 1;
  if (a)
...
{ register struct soap_plist *pp;
  *ppp = NULL;
  ...
  { for (pp = soap->pht[soap_hash_ptr(p)]; pp; pp = pp->next)
    {
      //SOAP_DPrint("[%s] pp->type:%d, type:%d\n", __FUNCTION__, pp->type, type);//linjieling
      if (pp->ptr == p && pp->type == type)
      { *ppp = pp;
        //SOAP_DPrint("[%s] Lookup location=%p type=%d id=%d\n", __FUNCTION__,  p, type, pp->id);//linjieling
        DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Lookup location=%p type=%d id=%d\n", p, type, pp->id));
        return pp->id;
      }
    }
  }
  //SOAP_DPrint("[%s] Lookup location=%p type=%d: not found\n", __FUNCTION__, p, type);//linjieling
...
{ struct soap_plist *pp = NULL;
  //SOAP_DPrint("[%s] --->\n", __FUNCTION__);//linjieling
  //SOAP_DPrint("[%s] Embedded_id %p type=%d id=%d\n", __FUNCTION__, p, t, id);//linjieling
...
# endif
...
#endif
    copy->plugins = NULL;
    //SOAP_DPrint("[%s] soap->plugins:%p\n", __FUNCTION__, soap->plugins);//linjieling
    ...
    { register struct soap_plugin *q = (struct soap_plugin*)SOAP_MALLOC(copy, sizeof(struct soap_plugin));
      ...
      *q = *p;
      ...
      q->next = copy->plugins;
      //SOAP_DPrint("[%s] q->next:%p\n", __FUNCTION__, q->next);//linjieling
      copy->plugins = q;
      //SOAP_DPrint("[%s] copy->plugins:%p\n", __FUNCTION__, copy->plugins);//linjieling
    }
  }
  //SOAP_DPrint("[%s] <---\n", __FUNCTION__);//linjieling
  return copy;
...
{
  SOAP_DPrint("[%s] state:%d\n", __FUNCTION__, soap->state);
  if (soap_check_state(soap))
    return;
  soap_free_temp(soap);
...
soap_set_local_namespaces(struct soap *soap)
{
  ...
  { register const struct Namespace *ns1;
    ...
    { memcpy(ns2, soap->namespaces, n);
      if (ns2[0].ns)
      {
        //SOAP_DPrint("[%s] ns[0].ns:%s\n", __FUNCTION__, ns2[0].ns);//linjieling
...
{
    //SOAP_DPrint("[%s] --->\n", __FUNCTION__);//linjieling
    //SOAP_DPrint("[%s] Element begin tag='%s' level='%u' id='%d' type='%s'\n", __FUNCTION__, 
    //    tag, soap->level, id, type ? type : SOAP_STR_EOS);//linjieling
  ...
  //  SOAP_DPrint("[%s] encoding:%d, encodingStyle:%s, local_namespaces:%s\n", __FUNCTION__, 
  //  soap->encoding, soap->encodingStyle, soap->local_namespaces[1].out);//linjieling
  soap->encoding = 0;
  if (soap->encoding)
...
{
    //SOAP_DPrint("[%s] --->\n", __FUNCTION__);//linjieling
    //SOAP_DPrint("[%s] Element ending tag='%s'\n", __FUNCTION__, tag);//linjieling
...
{
    //SOAP_DPrint("[%s] --->\n", __FUNCTION__);//linjieling
    //SOAP_DPrint("[%s] Attribute '%s'='%s'\n", __FUNCTION__, name, value);//linjieling
...
{ register struct soap_attribute *tp;
  SOAP_DPrint("[%s] --->\n", __FUNCTION__);//linjieling
  SOAP_DPrint("[%s] Set attribute %s='%s'\n", __FUNCTION__, name, value ? value : SOAP_STR_EOS);//linjieling
...
{ id = soap_element_id(soap, tag, id, *p, NULL, 0, type, n);
  ...
  if (id < 0)
    return soap->error;
  //SOAP_DPrint("[%s]soap_outstring get *p is *p:%p\n", __FUNCTION__, *p);
  //SOAP_DPrint("[%s]soap_outstring get **p is **p:%d\n", __FUNCTION__, **p);
...
{ register soap_wchar c;
  ...
  *soap->endpoint = '\0';
    ...
    { c = soap_get1(soap);
      ...
      { soap->mode &= ~SOAP_ENC_LATIN;
        ...
        c = (0x0F << 12) | (0xBB << 6) | (c & 0x3F); /* UTF-8 */
          ...
          || (c == 0xFF && soap_get0(soap) == 0xFE)){ /* UTF-16 LE */
      SOAP_DPrint("[%s] error5:%d\n", __FUNCTION__, SOAP_UTF_ERROR);//linjieling
      return soap->error = SOAP_UTF_ERROR;
    }
    /* skip space */
    while (soap_blank(c))
  ...
  if ((int)c == EOF){
    SOAP_DPrint("[%s] error6:%d\n", __FUNCTION__, SOAP_CHK_EOF);//linjieling
    return soap->error = SOAP_CHK_EOF;
  }
  soap_unget(soap, c);
#ifndef WITH_NOHTTP
  ...
  { soap_mode m = soap->imode;
    ...
    { if (soap->status == 200){
        SOAP_DPrint("[%s] error8:%d\n", __FUNCTION__, SOAP_NO_DATA);//linjieling
        return soap->error = SOAP_NO_DATA; /* HTTP OK: always expect data */
      }
      SOAP_DPrint("[%s] error9:%d\n", __FUNCTION__, soap->status);//linjieling
...
#endif
  //SOAP_DPrint("[%s] tmpbuf:%s\n", __FUNCTION__, soap->tmpbuf);//linjieling
