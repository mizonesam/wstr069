...
int main(int argc, char *argv[])
{
	int i = 0;
	cJSON *json = NULL;
	char str[] = {"{\"action\":\"gatherInfo\", \"mountPath\":\"/mount/staorage/URJTTT1\", \"time\":\"10\"}"};
	char *tests[3] = {"/action", "/mountPath", "/time"};
	json = cJSON_Parse(str);
	for (i=0; i<3; i++){
		char *output = cJSON_Print(cJSONUtils_GetPointer(json,tests[i]));
		printf("%s : %s\n", tests[i] + 1, output);
		free(output);
	}
	cJSON_Delete(json);
	...
	cJSON_AddItemToObject(root, "state", cJSON_CreateString("true"));
	cJSON_AddItemToObject(root, "errno", cJSON_CreateString("null"));
	cJSON_AddItemToObject(root, "showstream", cJSON_CreateString("null"));
	cJSON_AddItemToObject(root, "showott", cJSON_CreateString("null"));
	char *print = cJSON_Print(cJSONUtils_GetPointer(root, ""));
	printf("\n\nsendMsg:\n%s\n", print);
	free(print);
	print = NULL;
	cJSON_Delete(root);
